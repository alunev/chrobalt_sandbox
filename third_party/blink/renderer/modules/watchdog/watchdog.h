...
//  base::ApplicationState state_ = base::kApplicationStateStarted;
  ...
  std::unordered_map<std::string, std::unique_ptr<Client>> client_map_ ALLOW_DISCOURAGED_TYPE("migrating from Cobalt");
  // List of client registered Watchdog clients, parallel data structure to
  // client_map_.
  std::vector<std::shared_ptr<Client>> client_list_ ALLOW_DISCOURAGED_TYPE("migrating from Cobalt");
