...
//const int kSbFileMaxPath = 128;
//
//const char kSbSystemPathCacheDirectory[] = "~/test-cache";
//const char kSbFileSepString[] = "/";

}  // namespace

//WatchdogThread::WatchdogThread() {
//  LOG(INFO) << "WatchdogThread::WatchdogThread()";
////  Initialize();
//}

bool WatchdogThread::Initialize() {
  LOG(INFO) << "WatchdogThread::Initialize()";
  return InitializeCustom(
      std::string(kWatchdogViolationsJson),
      kWatchdogMonitorFrequency);
}

bool WatchdogThread::InitializeCustom(
  ...
  int res =
      pthread_create(&watchdog_thread_,
                     nullptr,
                     &WatchdogThread::Monitor,
                     this);

  LOG(INFO) << "  pthread_create: " << res;

  return true;
}

bool WatchdogThread::InitializeStub() {
  is_disabled_ = true;
  return true;
}

void WatchdogThread::Uninitialize() {
...
std::shared_ptr<base::Value> WatchdogThread::GetViolationsMap() {
...
std::string WatchdogThread::GetWatchdogFilePath() {
...
std::vector<std::string> WatchdogThread::GetWatchdogViolationClientNames() {
...
//void WatchdogThread::UpdateState(base::ApplicationState state) {
//  if (is_disabled_) return;
//
//  base::AutoLock scoped_lock(mutex_);
//  state_ = state;
...
void WatchdogThread::WriteWatchdogViolations() {
  ...
  LOG(INFO) << "[Watchdog] skipping write in prototype\n";
...
void* WatchdogThread::Monitor(void* context) {
  pthread_setname_np(pthread_self(), "Watchdog");
  base::AutoLock scoped_lock(static_cast<WatchdogThread*>(context)->mutex_);
  while (1) {
    ...
    for (auto& it: static_cast<WatchdogThread*>(context)->client_map_) {
    ...
    for (auto& it: static_cast<WatchdogThread*>(context)->client_list_) {
    ...
    if (static_cast<WatchdogThread*>(context)->pending_write_)
    ...
    base::TimeDelta timeout = base::Microseconds(
        static_cast<WatchdogThread*>(context)->watchdog_monitor_frequency_);
    static_cast<WatchdogThread*>(context)->monitor_wait_.TimedWait(timeout);

    // Shutdown
    if (!(static_cast<WatchdogThread*>(context)->is_monitoring_.load())) break;
  }
  return nullptr;
}

bool WatchdogThread::MonitorClient(void* context, Client* client,
                                   int64_t current_monotonic_time) {
...
//  if (static_cast<WatchdogThread*>(context)->state_ > client->monitor_state) {
...
//    return false;
//  }
  LOG(INFO) << "WatchdogThread::MonitorClient()";
...
void WatchdogThread::UpdateViolationsMap(void* context, Client* client,
                                         int64_t time_delta) {
  LOG(INFO) << "WatchdogThread::UpdateViolationsMap()";

  // Gets violation dictionary with key client name from violations map.
  WatchdogThread* watchdog_instance = static_cast<WatchdogThread*>(context);
  ...
  if (violation_dict == nullptr) {
    ...
    if (violations) {
      int last_index = static_cast<int>(violations->size()) - 1;
...
//                         GetApplicationStateString(client->monitor_state))));
  ...
  auto violation_map =
      (static_cast<WatchdogThread*>(context)->GetViolationsMap())->GetDict().Clone();
  for (base::Value::Dict::iterator it = violation_map.begin();
       it != violation_map.end(); ++it) {
    base::Value& local_violation_dict = it->second;
    base::Value::List* violations =
        local_violation_dict.GetDict().FindList("violations");
    if (violations) {
...
void WatchdogThread::EvictWatchdogViolation(void* context) {
  ...
  for (base::Value::Dict::iterator it = violation_map.begin();
    ...
    if (violations) {
      int count = static_cast<int>(violations->size());
      std::string* timestamp_duration_milliseconds =
  ...
  auto* violation_dict =
      (static_cast<WatchdogThread*>(context)->GetViolationsMap())
          ->GetDict()
  ...
  if (violation_dict != nullptr) {
    ...
    if (violations) {
      ...
      static_cast<WatchdogThread*>(context)->pending_write_ = true;
      ...
      if (static_cast<WatchdogThread*>(context)->GetViolationsMap()->DictSize()
          ==
              0) {
//        starboard::SbFileDeleteRecursive(
//            static_cast<WatchdogThread*>(context)->GetWatchdogFilePath().c_str(),
//            true);
        static_cast<WatchdogThread*>(context)->pending_write_ = false;
      }
    }
  }
}

void WatchdogThread::MaybeWriteWatchdogViolations(void* context) {
  if ((base::TimeTicks::Now() - base::TimeTicks()).InMicroseconds() >
      static_cast<WatchdogThread*>(context)->time_last_written_microseconds_ +
          static_cast<WatchdogThread*>(context)->write_wait_time_microseconds_) {
    static_cast<WatchdogThread*>(context)->WriteWatchdogViolations();
  }
}

void WatchdogThread::MaybeTriggerCrash(void* context) {
  if (static_cast<WatchdogThread*>(context)->GetPersistentSettingWatchdogCrash()) {
    if (static_cast<WatchdogThread*>(context)->pending_write_)
      static_cast<WatchdogThread*>(context)->WriteWatchdogViolations();
...
bool WatchdogThread::Register(std::string name, std::string description,
                              int64_t time_interval_microseconds,
                              int64_t time_wait_microseconds, Replace replace) {
  LOG(INFO) << "WatchdogThread::Register()";
  if (is_disabled_) return true;
  ...
  std::unique_ptr<Client> client = CreateClient(
      name, description, time_interval_microseconds,
...
std::shared_ptr<Client> WatchdogThread::RegisterByClient(
...
std::unique_ptr<Client> WatchdogThread::CreateClient(
    std::string name, std::string description,
    int64_t time_interval_microseconds,
...
bool WatchdogThread::Unregister(const std::string& name, bool lock) {
  if (is_disabled_) return true;
  ...
  if (lock) {
    ...
    result = static_cast<int>(client_map_.erase(name));
  } else {
    result = static_cast<int>(client_map_.erase(name));
  }
...
bool WatchdogThread::UnregisterByClient(std::shared_ptr<Client> client) {
  if (is_disabled_) return true;
...
bool WatchdogThread::Ping(const std::string& name) { return Ping(name, ""); }

bool WatchdogThread::Ping(const std::string& name, const std::string& info) {
  if (is_disabled_) return true;
...
bool WatchdogThread::PingByClient(std::shared_ptr<Client> client) {
...
bool WatchdogThread::PingHelper(Client* client, const std::string& name,
                                const std::string& info) {
  // Validates parameters.
...
std::string WatchdogThread::GetWatchdogViolations(
  ...
  LOG(INFO) << "WatchdogThread::GetWatchdogViolations()";
...
//        starboard::SbFileDeleteRecursive(GetWatchdogFilePath().c_str(), true);
      ...
      for (std::string name: clients) {
        ...
        if (violation_dict != nullptr) {
          LOG(INFO) << "setting fetched violations";
      ...
      LOG(INFO) << "fetched_violations.size(): " << fetched_violations.size();
...
void WatchdogThread::EvictOldWatchdogViolations() {
  int64_t current_timestamp_millis =
      (base::Time::Now() - base::Time::UnixEpoch()).InMilliseconds();
...
bool WatchdogThread::GetPersistentSettingWatchdogEnable() {
  return true;
}

void WatchdogThread::SetPersistentSettingWatchdogEnable(bool enable_watchdog) {
}

bool WatchdogThread::GetPersistentSettingWatchdogCrash() {
  return false;
}

void WatchdogThread::SetPersistentSettingWatchdogCrash(bool can_trigger_crash) {
}

bool WatchdogThread::LogEvent(const std::string& event) {
...
std::vector<std::string> WatchdogThread::GetLogTrace() {
  if (is_logtrace_disabled_) {
    return {};
  }
...
bool WatchdogThread::GetPersistentSettingLogtraceEnable() {
  return true;
}

void WatchdogThread::SetPersistentSettingLogtraceEnable(bool enable_logtrace) {
}

//#if defined(_DEBUG)
//// Sleeps threads for Watchdog debugging.
//void WatchdogThread::MaybeInjectDebugDelay(const std::string& name) {
//  if (is_disabled_) return;
//
...
//    usleep(static_cast<int>(delay_sleep_time_microseconds_));
