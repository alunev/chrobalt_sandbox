...
//  DEFINE_STATIC_LOCAL(Watchdog, instance, ());
//  return instance;
//}
...
void Watchdog::UpdateViolationsMap(void* context, Client* client,
  ...
  auto violation_dict =
      std::shared_ptr<base::Value::Dict>((watchdog_instance->GetViolationsMap())->GetDict().FindDict(client->name));
...
void Watchdog::EvictWatchdogViolation(void* context) {
  ...
  auto violation_dict = std::shared_ptr<base::Value::Dict>((static_cast<Watchdog*>(context)->GetViolationsMap())
      ->GetDict()
...
//        starboard::SbFileDeleteRecursive(GetWatchdogFilePath().c_str(), true);
      ...
      for (std::string name : clients) {
        auto violation_dict =
            std::shared_ptr<base::Value::Dict>(GetViolationsMap()->GetDict().FindDict(name));
